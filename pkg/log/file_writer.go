...
func (w *FileWriter) CreateLogFile() error {
	...
	if file, err := os.OpenFile(w.filename,
		os.O_RDWR|os.O_CREATE|os.O_APPEND, 0664); err != nil {
		return err
	} else {
		w.file = file
	}
...
func convertPatternToFmt(pattern []byte) string {
	...
	pattern = bytes.Replace(pattern, []byte("%D"), []byte("%d"), -1)
	pattern = bytes.Replace(pattern, []byte("%H"), []byte("%02d"), -1)
	pattern = bytes.Replace(pattern, []byte("%m"), []byte("%d"), -1)
